...
class HeartDiseaseRawCentralised(Dataset):
...
class FedHeartDiseaseCentralised(HeartDiseaseRawCentralised):
    """
    ...
    def __init__(
        ...
        for c in center if type(center) == list else [center]:
            assert c in range(
                0, get_num_clients()), f"Must specify a valid center, number of clients is {get_num_clients()}"

        self.chosen_centers = center if type(center) == list else [center]
