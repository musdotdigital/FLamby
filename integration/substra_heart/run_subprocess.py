...
def main(model: List[int]):
    ...
    TRAINING_CENTER_STRING = ''.join(str(e) for e in model)
    MODEL_PATH = 'models/' + f'model_{TRAINING_CENTER_STRING}.pt'

    # model = fed_heart_disease.Baseline() if not client and model_available(MODEL_PATH) else torch.load(MODEL_PATH)
